...
- Get integration tests running against the local Postgres on this VPS; make `yarn test` green in both packages. **Server done 2026-09-10** (JO-10): nothing was broken, only undocumented — `server/.env.example` + `server/README.md` now describe the env, and the suite is green (108 unit + 107 integration as of JO-47) against local Postgres 18. Still open for `web`, whose `npm test` has one suite failing to compile (JO-53).
